<div class="relative" wire:click.outside="close">
    ...
    <button wire:click="toggle" class="flex items-center gap-2">
        ...
        @else
            <div 
                class="w-9 h-9 rounded-full bg-gradient-to-br from-[#EB5160] to-[#FFB347] 
    ...
    @if($open)
        <div class="absolute right-0 top-14 bg-white border border-[#FFB347]/50 rounded-xl shadow-lg w-44 overflow-hidden">
            <a href="{{ route('profile', auth()->user()->username) }}" 
               class="block px-4 py-3 text-sm hover:bg-[#FFF2DD]">👤 Profil Saya</a>
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="w-full text-left block px-4 py-3 text-sm text-[#EB5160] hover:bg-[#FFF2DD]">
                    🚪 Logout
                </button>
            </form>
        </div>
